...
================================================================
数据源: Mimiwftt (Mi姐/Mistery) 完整交易体系
...
def compute_signals(df: pd.DataFrame, m1_df: Optional[pd.DataFrame]) -> List[Signal]:
    ...
    if pd.notna(ma250) and ma250 > 0:
        ...
        if dev250 > 0.05:
            s = Signal("年线位置", f"年线上方 {dev250:+.1%}", "healthy",
                "指数站稳年线——这是牛市的必要条件。Mi姐铁律：日线只做所有均线都在250日线之上的个股。",
            ...
            s = Signal("年线位置", f"年线附近 {dev250:+.1%}", "caution",
                "指数在年线附近拉锯——方向不明。Mi姐说这种时候'宁可休息'。",
                "「趋势不明，宁可休息」—《趋势交易论》口诀第6条")
        else:
            s = Signal("年线位置", f"年线下方 {dev250:+.1%}", "danger",
                "指数在年线下方运行，长期趋势偏弱。Mi姐：年线之下重防守，不要重仓。",
    ...
    if golden_520:
        s = Signal("520战法", "MA5金叉MA20 ✅", "healthy",
            "520金叉——短期趋势转多。Mi姐的520战法核心信号：金叉买入，持股待涨。",
    ...
    if rsi > RSI_EXTREME_OVERBOUGHT:
        s = Signal("RSI", f"RSI={rsi:.0f} 极端超买", "danger",
            "RSI>80=极端超买。Mi姐：高潮=风险，'高潮减仓'。这时候不是追高的时机。",
        ...
        s = Signal("RSI", f"RSI={rsi:.0f} 极端超卖", "caution",
            "RSI<20=极端恐慌。Mi姐：冰点是希望的开始。但不急于抄底——等底部确认信号（底分型+金叉+放量）。",
    ...
    if monthly == "bull" and weekly == "bull" and daily == "↑":
        s = Signal("三周期趋势", f"{mtf_val} — 共振向上 ✅", "healthy",
            "月线、周线、日线三级别全部向上——这是最强的做多信号！Mi姐的三周期选股法：大周期定方向、中周期定结构、小周期抓节奏。三个级别共振=可以重仓。",
    ...
    if vr > VOL_STAGNANT and abs(chg) < 0.003:
        s = Signal("量价结构", f"放量滞涨 (vol={vr:.1f}x) ⚠", "danger",
            "放巨量但价格基本不动——Mi姐说这是最危险的信号！往往是主力在高位出货。历史上多次大顶前都出现过放量滞涨。",
...
def ai_interpret(cycle: Dict, signals: List[Signal], sectors: List[Dict],
    ...
    try:
        ...
        prompt = f"""你是Mi姐（Mimiwftt/Mistery），《趋势交易论》作者，直言不讳的A股趋势交易者。
...
要求：直接、接地气、不模棱两可、不用分析师语言。就像在跟朋友聊天。"""
        ...
        resp = requests.post(
            ...
            json={"model": "deepseek-chat", "messages": [
                {"role": "system", "content": "你是Mi姐（Mimiwftt/Mistery），《趋势交易论》作者，A股趋势交易者。说话直接、接地气、不装腔作势。常用口头禅：买横买坑不买竖、趋势不明宁可休息、高位放量滞涨坚决出局。"},
...
def send_feishu(content: str) -> bool:
    ...
    card = {
        ...
        "header": {"template": "blue", "title": {"tag": "plain_text", "content": "A股全景仪表盘 · Mi姐框架"}},
