...
当输入中包含「📊 A股板块异动监测」数据时，你必须将其纳入分析。这些数据来自一份专业会议的技术框架，已经预计算了各板块的技术面信号和资金流向。

**板块分类框架说明：**
- 📥 ENTRY（可入场）= 会议认为可低吸/抄底的板块
- 🟢 HOLD（持有）= 趋势完好但不可开新仓，只能持有
- 🟡 WATCH（观察）= 接近满足条件但需进一步确认
- 🔴 AVOID（回避）= 下降趋势/弱势，不入场

**你需要做的事：**

1. **对照会议框架，确认或质疑每个板块的信号。** 例如：某个 ENTRY 板块今日出现了放量下跌 → 质疑其入场信号是否仍然有效。某个 AVOID 板块出现了意外放量阳线突破 → 提示是否正在反转。

2. **在 key_changes 中记录板块级别的实质变化。** 例如：底分型确认、放量站上5日线、资金从持续流出转为流入。

3. **在 opportunity_ranking 中输出A股板块级别的机会。** 使用会议框架的类别（机器人方向/消费/科技/医药/周期/金融/新能源/制造/公用事业/物流），标注 signal_strength 和可证伪条件。

4. **在 cross_sectional_patterns 中寻找跨板块模式。** 例如：多个消费板块同时出现底部放量 → 消费板块可能整体见底。科技与证券跷跷板 → 资金轮动信号。

5. **关注资金异动板块。** 如果某个板块的5日主力资金流与会议框架预期方向相反，标记为潜在预期差。

**A股板块与美股机会的关系：** 如果A股某个板块出现强信号，检查是否有对应的美股标的（如中概股、相关ETF），在 opportunity_ranking 中并列输出。
...
def build_ai_input(items: List[Dict], fred_items: List[Dict], akshare_text: str, polymarket_markets: List[Dict] = None, active_theses: List[Dict] = None, calibration_stats: Dict = None, tavily_items: List[Dict] = None, sector_text: str = "") -> str:
    # 按类别分组
    cats = {}
    for item in items:
...
def main():
    ...
    if _HAS_SECTOR_MONITOR:
        sm_config = sources_config.get("sector_monitor", {}) if isinstance(sources_config, dict) else {}
        if sm_config.get("enabled"):
            try:
                sector_data = fetch_sector_monitor_data()
                sector_text = format_sector_for_prompt(sector_data)
                log(f"📊 板块监测: {sector_data.get('summary',{}).get('entry_count',0)}入场 "
                    f"{sector_data.get('summary',{}).get('avoid_count',0)}回避")
            except Exception as e:
                log(f"⚠️ 板块监测错误: {str(e)[:80]}")
    ...
    ai_input = build_ai_input(filtered, fred_items, akshare_text, fetch_polymarket_data(), load_active_theses(), get_calibration_stats(), tavily_items, sector_text)
