...
def test_card_valuation_partial_failure_note():
    ...
    card = format_card("2026-08-04", "解读文本", focus, southbound, valuation, repurchase, refs, quarterly_expired=False)
    assert "银行" in card
    assert "另有 2 个板块当日估值源不可用" in card
    card_all_fail = format_card("2026-08-04", "解读文本", *_args(), quarterly_expired=False)
